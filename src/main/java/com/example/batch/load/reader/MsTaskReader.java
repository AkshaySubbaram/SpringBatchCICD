...
@Component
@StepScope
public class MsTaskReader implements ItemReader<MsTasks> {

    private static final Logger logger = LoggerFactory.getLogger(MsTaskReader.class);
    ...
    public MsTaskReader(@Value("${input.file.msTask}") Resource resource) throws IOException {
        this.resource = resource;
        logger.info("MsTask File exists: {}", resource.exists());
        logger.info("MsTask File is readable: {}", resource.isReadable());
        logger.info("MsTask Absolute path: {}", resource.getFile().getAbsolutePath());
    }

    @Override
    public MsTasks read() throws Exception {
        logger.info("inside task read");
        if (rowIterator == null) {
            initialize();
            currentRow = 0;
        }
        ...
        logger.info("READING ROW MsTask {}: {}", currentRow, rowToString(row));
    ...
    private MsTasks mapRowToSkill(Row row) {
        MsTasks tasks = new MsTasks();

        tasks.setTaskid(getIntValue(row.getCell(0)));
        tasks.setTasktext(getStringValue(row.getCell(1)));
        tasks.setTaskoverview(getStringValue(row.getCell(2)));
        tasks.setTaskdescription(getStringValue(row.getCell(3)));
        tasks.setTaskconclusion(getStringValue(row.getCell(4)));
        tasks.setLastupdated(getStringValue(row.getCell(5)));
